...
- flowing:
  - data 이벤트에 리스너를 등록
  - read()를 사용하여 꺼내지 않고 데이터가 도착하자마자 리스너에 전달
  - 이전 버전의 스트림 인터페이스(Stream1)의 상속
  - Stream2 인터페이스의 도입으로 기본 작동 모드 아님
  - data 이벤트 리스너 등록, resume() 메소드를 명시적 호출
  - 이벤트 발생 일시적으로 중지 위해서는 pause() 메소드 호출, 데이터는 내부 버퍼에 캐시
